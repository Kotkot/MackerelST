...
######## unused bit to keep just in case
no_used <- function(){
  if (conf$Model_type == "Normal") {
    ...
    Map_phase1 <- list()
    Map_phase1$beta <- matrix(1:(ncol(data$tmb_data$X)*data$tmb_data$Nage), nrow=ncol(data$tmb_data$X), ncol= data$tmb_data$Nage, byrow=F)  
    Map_phase1$beta <- factor(Map_phase1$beta)         # To deal with estimability due to scaling 
    Map_phase1$logTauE <- factor(rep(NA, length(data$tmb_params$logTauE)))
    Map_phase1$epsilon_st <- factor(rep(NA, length(data$tmb_params$epsilon_st)))
    Map_phase1$transf_rho <- factor(NA)
    Map_phase1$epsilon_cov <- factor(rep(NA, length(data$tmb_params$epsilon_cov)))
    Map_phase1$epsilon_cov_proj <- factor(rep(NA, length(data$tmb_params$epsilon_cov_proj)))
    if(conf$keep_omega == TRUE) cli_warn(c("keep_omega = FALSE is the only option that is available for now"))
    if (conf$keep_omega == FALSE) {
      Map_phase1$omega <- factor(rep(NA, length(data$tmb_params$omega)))
      Map_phase1$logTauO <- factor(rep(NA, length(data$tmb_params$logTauO)))
      Map_phase1$logKappa <- factor(rep(NA, length(data$tmb_params$logKappa)))
      tmb_obj_phase1 <- TMB::MakeADFun(data = data$tmb_data, parameters = data$tmb_params, map = Map_phase1,
                                       random = NULL, DLL = "spatial_SDM_RE", silent = FALSE)
    }
    if (keep_omega == TRUE) {
      tmb_obj_phase1 <- TMB::MakeADFun(data = data$tmb_data, parameters = data$tmb_params, map = Map_phase1,
                                       random = "omega", DLL = "spatial_SDM_RE", silent = FALSE)
    }
  ...
  if (conf$Model_type == "Normal") {
    ...
    #Map_phase1$logTauO <- factor(rep(NA, length(data$tmb_params$logTauO)))
    # Map_phase2$epsilon_st <- factor(rep(NA, length(data$tmb_params$epsilon_st)))
    Map_phase2$transf_rho <- factor(NA)
    Map_phase2$epsilon_cov <- factor(rep(NA, length(data$tmb_params$epsilon_cov)))
    Map_phase2$epsilon_cov_proj <- factor(rep(NA, length(data$tmb_params$epsilon_cov_proj)))
    # Map_phase2$omega <- factor(rep(NA, length(data$tmb_params$omega)))
    # Map_phase2$logTauO <- factor(rep(NA, length(data$tmb_params$logTauO)))
    # Map_phase2$logKappa <- factor(rep(NA, length(data$tmb_params$logKappa)))
    # Map_phase2$thetaf <- factor(rep(10000, length(data$tmb_params$thetaf)))
    # Map_phase2$ln_phi <- factor(rep(20000, length(data$tmb_params$ln_phi)))
    ...
    if (keep_omega == FALSE) {
      ...
      tmb_obj_phase2 <- TMB::MakeADFun(data = data$tmb_data, parameters = data$tmb_params, map = Map_phase2,
                                       random = c("epsilon_st"), DLL = "spatial_SDM_RE", silent = FALSE)
    }
    if (keep_omega == TRUE) {
      data$tmb_params$beta <- matrix(set_par_value(opt_phase1, "beta"), nrow=ncol(data$tmb_data$X), ncol= data$tmb_data$Nage, byrow=F)   # making sure to keep at 0 the age1 groups fixed effects
      data$tmb_params$logTauO <- set_par_value(opt_phase1, "logTauO")
      ...
      tmb_obj_phase2 <- TMB::MakeADFun(data = data$tmb_data, parameters = data$tmb_params, map = Map_phase2,
                                       random = c("omega", "epsilon_st"), DLL = "spatial_SDM_RE", silent = FALSE)
    }
  ...
  fit_mackerelST_condlogit_old <- function(data, conf, conf_extra=NULL) {
    ...
    Map_phase1 <- list()
    if (!is.null(attributes(conf)$fixed_effect)){
      Map_phase1$beta <- matrix(1:(ncol(data$tmb_data$X)*(data$tmb_data$Nage-1)), nrow=ncol(data$tmb_data$X), ncol= data$tmb_data$Nage-1, byrow=F)  
      Map_phase1$beta <- factor(Map_phase1$beta)        
    }
    Map_phase1$epsilon_st <- factor(rep(NA, length(data$tmb_params$epsilon_st)))
    Map_phase1$logTauE <- factor(rep(NA, length(data$tmb_params$logTauE)))
    ...
    Map_phase1$logslope <- factor(NA)
    Map_phase1$transf_rho <- factor(rep(NA, length(data$tmb_params$transf_rho)))
    Map_phase1$transf_rho_cpue <- factor(rep(NA, length(data$tmb_params$transf_rho_cpue)))
    # Map_phase1$logKappa <- factor(rep(NA, length(data$tmb_params$logKappa)))
    Map_phase1$logKappa <- factor(rep(4321, length(data$tmb_params$logKappa)))
    Map_phase1$ln_tau_G <- factor(rep(1234,length(data$tmb_params$ln_tau_G)))
    ...
    # random_var = c("RE_diff")
    random_var = c("RE_diff","init_year","init_age")
    if (conf$keep_epsilon == TRUE) {
      random_var = c(random_var, "epsilon_st_cpue")
    }
    ...
    if (conf$keep_omega == FALSE) {
      Map_phase1$omega <- factor(rep(NA, length(data$tmb_params$omega)))
      Map_phase1$omega_cpue <- factor(rep(NA, length(data$tmb_params$omega_cpue)))
      # Map_phase1$logTauO <- factor(rep(NA, length(data$tmb_params$logTauO)))
      if (conf$keep_epsilon == FALSE) {
        Map_phase1$epsilon_st_cpue <- factor(rep(NA, length(data$tmb_params$epsilon_st_cpue)))
        Map_phase1$logTauE_cpue <- factor(rep(NA, length(data$tmb_params$logTauE_cpue)))
      }
      if (FALSE %in% is.na(attributes(conf)$RE_effects))  random_var = c(random_var, "RE")
      if (FALSE %in% is.na(attributes(conf)$RE_effects_cpue))  random_var = c(random_var, "RE_cpue")
      if (TRUE %in% is.na(attributes(conf)$RE_effects)) {
      ...
      if (TRUE %in% is.na(attributes(conf)$RE_effects_cpue)) {
        Map_phase1$RE_cpue <- factor(rep(NA,length(data$tmb_params$RE_cpue)))
        Map_phase1$ln_tau_G_cpue <- factor(rep(NA,length(data$tmb_params$ln_tau_G_cpue)))
      }  
      tmb_obj_phase1 <- TMB::MakeADFun(data = data$tmb_data, parameters = data$tmb_params, map = Map_phase1,
                                       random = random_var, DLL = dllversion, silent = TRUE)
    }
    
    # tmb_obj_phase1$fn(tmb_obj_phase1$par)
    opt_phase1 <- fit_tmb(tmb_obj_phase1, lower=-15, upper=15, getsd=FALSE, bias.correct=FALSE, control = list(eval.max = 20000, iter.max = 20000, trace = TRUE))          
    # opt_phase1 <- nlminb(start=tmb_obj_phase1$par, objective = tmb_obj_phase1$fn, gradient = tmb_obj_phase1$gr, lower=-15, upper=15)
    ...
    ### Step2: Putting the spatio-temporal component into the estimation on the P(age) model
    if (conf$keep_epsilon == TRUE) {
      Map_phase2 <- list()
      if (!is.null(attributes(conf)$fixed_effect)){
        Map_phase2$beta <- matrix(1:(ncol(data$tmb_data$X)*(data$tmb_data$Nage-1)), nrow=ncol(data$tmb_data$X), ncol= data$tmb_data$Nage-1, byrow=F)  
        Map_phase2$beta <- factor(Map_phase2$beta)         
      }
      Map_phase2$logTauE <- factor(rep(30000, length(data$tmb_params$logTauE)))  # to simplify something
      
      Map_phase2$logKappa <- factor(rep(4321, length(data$tmb_params$logKappa)))
      # Map_phase2$logKappa_cpue <- factor(rep(4321, length(data$tmb_params$logKappa_cpue))) # putting the same as Kappa
      
      Map_phase2$s50 <- factor(NA)
      Map_phase2$logslope <- factor(NA)
      
      if (conf$ARorIID_cpue != 1)  Map_phase2$transf_rho_cpue <- factor(rep(NA, length(data$tmb_params$transf_rho_cpue)))
      if (conf$ARorIID != 1) Map_phase2$transf_rho <- factor(rep(NA, length(data$tmb_params$transf_rho)))
      ...
      # random_var = c("RE_diff", "epsilon_st", "epsilon_st_cpue")
      random_var = c("RE_diff","init_year","init_age", "epsilon_st", "epsilon_st_cpue")
      
      if (conf$keep_omega == FALSE) {
        
        Map_phase2$omega <- factor(rep(NA, length(data$tmb_params$omega)))
        Map_phase2$omega_cpue <- factor(rep(NA, length(data$tmb_params$omega_cpue)))
        ...
        if (TRUE %in% is.na(attributes(conf)$RE_effects_cpue)) {
          Map_phase2$RE_cpue <- factor(rep(NA,length(data$tmb_params$RE_cpue)))
          Map_phase2$ln_tau_G_cpue <- factor(rep(NA,length(data$tmb_params$ln_tau_G_cpue)))
        }  
        ...
        if (!is.null(attributes(conf)$fixed_effect)){
          ...
          map_beta <- Map_phase2$beta
          map_beta <- factor(map_beta, labels = seq(1, sum(!is.na(map_beta))))		
          data$tmb_params$beta <- matrix(old_par[map_beta], nrow=ncol(data$tmb_data$X), ncol= data$tmb_data$Nage-1, byrow=F)   
        }  
        data$tmb_params$ln_phi_cpue <- set_par_value(opt_phase1$par, "ln_phi_cpue")   
        data$tmb_params$thetaf_cpue <- set_par_value(opt_phase1$par, "thetaf_cpue")   
        data$tmb_params$ln_tau_G_cpue <- set_par_value(opt_phase1$par, "ln_tau_G_cpue")   
        # data$tmb_params$logKappa_cpue <- set_par_value(opt_phase1$par, "logKappa_cpue")   
        data$tmb_params$logKappa <- rep(set_par_value(opt_phase1$par, "logKappa"), length(data$tmb_params$logKappa))
        data$tmb_params$beta_cpue <- set_par_value(opt_phase1$par, "beta_cpue")   
        data$tmb_params$init_year <- set_par_value(tmb_obj_phase1$env$last.par.best, "init_year")   
        data$tmb_params$init_year_mean <- set_par_value(opt_phase1$par, "init_year_mean")
        data$tmb_params$init_year_SD <- set_par_value(opt_phase1$par, "init_year_SD")
        data$tmb_params$init_age <- set_par_value(tmb_obj_phase1$env$last.par.best, "init_age")   
        data$tmb_params$init_age_SD <- set_par_value(opt_phase1$par, "init_age_SD")
        data$tmb_params$RE_diff <- set_par_value(tmb_obj_phase1$env$last.par.best, "RE_diff")    
        data$tmb_params$logcohort_SD <- set_par_value(opt_phase1$par, "logcohort_SD")   
        if (FALSE %in% is.na(attributes(conf)$RE_effects)) {
          data$tmb_params$ln_tau_G <- matrix(rep(set_par_value(opt_phase1$par, "ln_tau_G"),(data$tmb_data$Nage-1)), ncol= data$tmb_data$Nage-1, byrow=F)   
          data$tmb_params$RE <- matrix(set_par_value(tmb_obj_phase1$env$last.par.best, "RE"), nrow=data$tmb_data$nobs_RE, ncol= data$tmb_data$Nage-1, byrow=F)    
        }
        data$tmb_params$epsilon_st_cpue <- matrix(set_par_value(tmb_obj_phase1$env$last.par.best, "epsilon_st_cpue"), nrow=data$tmb_data$Nmesh, ncol= data$tmb_data$Nyear, byrow=F)    
        data$tmb_params$logTauE_cpue <- set_par_value(opt_phase1$par, "logTauE_cpue")  
        data$tmb_params$RE_cpue <- set_par_value(tmb_obj_phase1$env$last.par.best, "RE_cpue")  
